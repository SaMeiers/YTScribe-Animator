...
function showAlert(message) {
    ...
    alert.innerHTML = `
        <h3>${langData.warning || 'Warning'}</h3>
        <p>${message}</p>
        <div class="custom-alert-buttons">
            <button class="custom-alert-btn custom-alert-btn-primary" onclick="this.parentElement.parentElement.remove()">${langData.ok || 'OK'}</button>
        </div>
    `;
...
function showPrompt(message, defaultValue, callback) {
    ...
    prompt.innerHTML = `
        ...
        <div class="custom-alert-buttons">
            <button class="custom-alert-btn custom-alert-btn-secondary" onclick="this.parentElement.parentElement.remove()">${langData.cancel || 'Cancel'}</button>
            <button class="custom-alert-btn custom-alert-btn-primary" id="customPromptConfirm">${langData.ok || 'OK'}</button>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', async function() {
    await loadLanguage(window.appLanguage || 'en');
    ...
    function handleCellClick(e) {
        ...
        cursorPosition.textContent = `${langData.position || 'Position'}: ${col},${row}`;
    ...
    function renderFrames() {
        ...
        appState.frames.forEach((frame, index) => {
            ...
            timeElement.addEventListener('dblclick', async () => {
                const newTime = await prompt(langData.newTime || 'New time (HH:MM:SS):', frame.time);
    ...
    function playAnimation() {
        ...
        if (window.playInterval) {
            ...
            playBtn.textContent = langData.play || 'Play';
        ...
        playBtn.textContent = langData.stop || 'Stop';
    ...
    function exportYTT() {
        if (appState.frames.length === 0) {
            alert(langData.addFrameFirst || 'Add at least one frame before exporting');
            return;
        }
